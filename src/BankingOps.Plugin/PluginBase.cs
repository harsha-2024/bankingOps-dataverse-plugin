...
namespace BankingOps.Plugin
{
    public abstract class PluginBase : IPlugin
    {
        ...
        { UnsecureConfig = unsecureConfig; SecureConfig = secureConfig; }
        ...
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            try { Execute(tracing, context, service, serviceProvider); }
            catch (InvalidPluginExecutionException) { throw; }
            catch (Exception ex)
            {
                tracing.Trace($"Unhandled exception: {ex}");
                throw new InvalidPluginExecutionException("BankingOps plugin error. See Plugin Trace Log.", ex);
            }
        }
